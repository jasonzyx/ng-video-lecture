...
eval_iters = 200 if PROD else 200
n_embd = 384 if PROD else 64
n_head = 6 if PROD else 6
n_layer = 6 if PROD else 6
dropout = 0.2
# ------------
...
# Replace character-level tokenization with GPT tokenizer
encoding = tiktoken.get_encoding("cl100k_base")  # This is the encoding used by GPT-4
vocab_size = encoding.n_vocab  # Much larger vocabulary (100k+ tokens vs char-level)

# Modify encode/decode functions to use tiktoken
...
model = train_model(m, optimizer, max_iters, eval_interval)

# First, let's find out what token IDs represent "Hi"
hi_tokens = encode("Hi")
print("Token IDs for 'Hi':", hi_tokens)

# Now use those tokens as context
context = torch.tensor([hi_tokens], dtype=torch.long, device=device)
print("Context as text:", decode(context[0].tolist()))

# Generate and print the output
generated = m.generate(context, max_new_tokens=500)[0].tolist()
print("\nGenerated text:", decode(generated))